...
<head>
    ...
    <title><?= $_SESSION["pelanggan"]["nama_user"] ?> || Profile Depot Istimewa Lawang</title>
</head>
    ...
    <div class="konten-web hal-profile">
        <div class="container">
            <div class="row justify-content-around">
                ...
                <!-- form profile akun -->
                <div class="col-lg-7">
                    <h3 class="color-secondary mb-4">PROFILE AKUN</h3>
                    <?php if (isset($_SESSION["alert_updateAkun"]["success"])) : ?>
                        <?php if ($_SESSION["alert_updateAkun"]["success"] === "Berhasil menyimpan perubahan profile") : ?>
                            <div class="alert alert-success alert-dismissible fade show" role="alert">
                                <strong><?= $_SESSION["alert_updateAkun"]["success"] ?></strong>
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if (isset($_SESSION["alert_updateAkun"]["fail"])) : ?>
                        <?php if ($_SESSION["alert_updateAkun"]["fail"] === "Password konfirmasi salah!") : ?>
                            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                <strong><?= $_SESSION["alert_updateAkun"]["fail"] ?></strong>
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>
                        <?php elseif ($_SESSION["alert_updateAkun"]["fail"] === "Email sudah terpakai") : ?>
                            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                                <strong><?= $_SESSION["alert_updateAkun"]["fail"] ?></strong>
                                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                    <form id="formUpdateProfile" action="index.php?page=pelanggan&aksi=konfirmasiProfile" method="post">
                        <div class="mb-3">
                            <label for="namaPelanggan" class="form-label">Nama Lengkap</label>
                            <input name="nama" type="text" class="form-control" id="namaPelanggan" placeholder="<?= $_SESSION["pelanggan"]["nama_user"] ?>" />
                        </div>
                        <div class="mb-3">
                            <label for="emailPelanggan" class="form-label">Email</label>
                            <input name="email" type="email" class="form-control" id="emailPelanggan" placeholder="<?= $_SESSION["pelanggan"]["email"] ?>" />
                        </div>
                        <div class="mb-3">
                            <label for="notelpPelanggan" class="form-label">Nomor Telepon/WA</label>
                            <input name="noHp" type="text" class="form-control" id="notelpPelanggan" placeholder="<?= $_SESSION["pelanggan"]["no_tlp"] ?>" />
                        </div>
                        <div class="mb-3">
                            <label for="alamatPelanggan" class="form-label">Alamat</label>
                            <input name="alamat" type="text" class="form-control" id="alamatPelanggan" placeholder="<?= $_SESSION["pelanggan"]["alamat"] ?>" />
                        </div>
                        <?php unset($_SESSION["alert_updateAkun"]); ?>
                        <div class="row justify-content-center mt-4">
                            <div class="col-md-6">
                                <div class="d-grid gap-2">
                                    <button disabled="disabled" id="simpanPerubahan" type="submit" class="btn-update btn color-primary-button mt-3 pt-2 fs-4">
                                        Simpan Perubahan
                                    </button>
                                </div>
                            </div>
